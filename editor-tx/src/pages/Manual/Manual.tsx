    ...
    sections: [
      {
        content: (
          <>
            <strong>Topo T16</strong> is a compact expressive MIDI controller. It is equipped with
            16 <strong>pressure-sensitive</strong> keys, capable of generating velocity and
            aftertouch messages. The touchstrip can be used to control useful parameters, like pitch
            bend and octave. The T16 is designed to be used with a computer via the{' '}
            <strong>USB C</strong> port, any hardware MIDI device using the{' '}
            <strong>MIDI TRS</strong> port, or using <strong>BLE</strong> to wirelessly connect to
            Android and iOS devices. Topo can do way more than just sending MIDI notes and can be
            configured to your needs using the simple and intuitive web interface.
          </>
        ),
      },
      {
      ...
      },
      {
        ...
        content: (
          <>
            The Web Editor allows you to customize many of Topo's features, like the keyboard layout
            or the CC and Channel mappings. Simply go to topo.unwn.dev and connect your device.
            Note: the editor uses WebMIDI to talk with your device, so you'll need to use Chrome or
            Chrome-based browsers (there is a WebMIDI extension for Firefox, but it is not reliable
            enough).
          </>
        ),
      },
    ...
    sections: [
      {
        content: (
          <>
            When in Keyboard mode, the T16 will output MIDI NoteOn/Off messages. Both the velocity
            and the aftertouch can be configured with your choice of reaction curve. The layout is
            fully customizable, allowing you to set a scale (chromatic by default), the root note
            and the direction to fit your playing style. To help you navigate the scale, root notes
            are highlighted on the grid using the key's RGB LEDs. In this mode the touchstrip has
        ...
        caption: 'The keyboard LEDs highlight each root note for the selected scale',
      },
      {
        content:
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        content: (
          <>
            To update the firmware of the device you can use Topo's update tool (accessible on the
            header, look for this icon: <MdOutlineSystemUpdateAlt className={styles.inlineIcon} />
            ), simply select the firmware version from the dropdown menu and follow the on-screen
            instructions.
          </>
        ),
      },
    ...
    <div className={styles.page}>
      ...
      <main className={styles.content}>
        {content.map((section, sectionIndex) => (
          <div key={sectionIndex} className={styles.section} ref={setSectionRef(sectionIndex)}>
            <h2 className={styles.sectionTitle}>{section.title}</h2>
            {section.sections.map((sub, subIndex) => (
              <div key={subIndex} className={styles.subSection}>
                {sub.subtitle && <h3 className={styles.subSectionTitle}>{sub.subtitle}</h3>}
                {sub.image ? (
                  <div className={styles.imageRow}>
                    ...
                    <div className={styles.imageRowMedia}>
                      ...
                      />
                      {sub.caption && <span className={styles.caption}>{sub.caption}</span>}
                    </div>
                  </div>
