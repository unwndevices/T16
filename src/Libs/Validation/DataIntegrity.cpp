...
uint32_t MessageValidator::computeChecksum(const void* data, size_t size, ChecksumType type) {
    switch (type) {
        case CRC8_BASIC:
        ...
        case REED_SOLOMON:
            // Reed-Solomon would require more complex implementation
            // For now, fallback to CRC32
            return crc32(data, size);
        case HASH_SHA256:
            // SHA256 would require crypto library
            // For now, fallback to CRC32
            return crc32(data, size);
        default:
            return 0;
